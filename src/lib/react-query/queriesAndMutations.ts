...
export const useCreatePost = () => {
  ...
  return useMutation({
    mutationFn: (post: INewPost) => createPost(post),  // Call the CreatePost function
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_RECENT_POSTS],  // This will trigger the refetch of the recent posts query
      });
    },
  });
};
...
export const useUpdatePost = () => {
  ...
  return useMutation({
    mutationFn: async (post: IUpdatePost) => {
      return await updatePost(post); // Ensure errors bubble up
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_POST_BY_ID, data?.$id],
...
export const useGetPosts = () => {
  return useInfiniteQuery({
    ...
    initialPageParam: undefined, // 👈 Fix: Explicitly set initialPageParam
